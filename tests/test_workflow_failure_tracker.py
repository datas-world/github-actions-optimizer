...
class TestWorkflowFailureTracker:
    ...
    def test_workflow_triggers_on_all_workflows(self, workflow_file):
        ...
        Implementation note: PyYAML automatically converts the YAML 'on' keyword
        to Python's boolean True when parsing. This is documented behavior:
        - https://yaml.org/type/bool.html (YAML 1.1 specification)
        - https://pyyaml.org/wiki/PyYAMLDocumentation
        - GitHub issue: https://github.com/yaml/pyyaml/issues/98

        The YAML boolean type includes: y|Y|yes|Yes|YES|n|N|no|No|NO|true|True|TRUE|false|False|FALSE|on|On|ON|off|Off|OFF
        ...
        """
        # Check both possible key representations due to PyYAML's boolean conversion
    ...
    def test_workflow_uses_correct_actions(self, workflow_file):
        ...
        for expected_action in expected_actions:
            ...
            # - https://docs.github.com/en/actions/creating-actions/about-custom-actions#using-versioning-in-actions
            # - GitHub repository naming: https://docs.github.com/en/repositories/creating-and-managing-repositories/about-repositories#repository-names
            #
            # Repository names cannot contain '@' characters per GitHub's naming rules.
            # The '@' symbol is reserved for version/reference specification in actions.
            # If multiple '@' symbols were present, it would indicate malformed action syntax.
            action_pattern = re.compile(
                r"^([a-zA-Z0-9\-_.]+/[a-zA-Z0-9\-_.]+)@([a-zA-Z0-9\-_.]+)$|"  # owner/repo@ref
            ...
            for action in actions_used:
                ...
                # Safe extraction with fail-safe for multiple @ symbols
                if "@" in action:
                    at_count = action.count("@")
                    if at_count > 1:
                        pytest.fail(f"Action contains multiple '@' symbols: {action}")
